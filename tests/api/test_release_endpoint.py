...
def test_create_release_with_no_signal(client):
    ...
    assert product_response['product']["artifact_url"] == product_details["artifact_url"]
...
def test_release_with_canary_period_with_all_bands_populated(client, db_session):
    ...
    send_signals_operation = lambda mock_datetime: send_signals(client, desired_timestamp, mock_datetime, 600, 100)
    results, _ = mock_date_and_perform_operation(desired_timestamp, send_signals_operation)
...
def send_signals(client, desired_timestamp, mock_datetime=None, loop_count=100, increment_condition=20):
    results = {}
...
def count_signals(db_session, release_id):
    return db_session.query(func.count(Signal.id)).join(
...
def test_release_created_with_signals_linked_to_release_bands(client, db_session):
    ...
    operation = lambda mock_datetime: create_release(client, new_release)
    second_release_id, mock_datetime = mock_date_and_perform_operation(desired_timestamp, operation)

    send_signals_operation = lambda mock_datetime: send_signals(client, desired_timestamp, mock_datetime)
    results, mock_datetime = mock_date_and_perform_operation(desired_timestamp, send_signals_operation)

    new_release["semver_version"] = "0.0.3"
    new_release["release_date"] = desired_timestamp.isoformat()

    operation = lambda mock_datetime: create_release(client, new_release)
    third_release_id, mock_datetime = mock_date_and_perform_operation(desired_timestamp, operation)
