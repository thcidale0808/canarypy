...
def test_client_get_latest_stable_version(canary_py_client):
    ...
    with patch.dict(os.environ, {"CANARYPY_URL": "http://test-url"}), \
         patch('requests.get', return_value=Mock(status_code=200, json=lambda: {"semver_version": "1.0.0"})) as mock_get:
...
def test_client_send_signal_to_canary(canary_py_client):
    ...
    with patch.dict(os.environ, {"CANARYPY_URL": "http://test-url"}), \
         patch('requests.post', return_value=Mock(status_code=200)) as mock_post:
        canary_py_client.send_signal_to_canary(artifact_url, version, instance_id, description, status)
