...
def test_integration_send_signal_to_canary(monkeypatch, client, runner, db_session):
    ...
    result = runner.invoke(release, ['create', '--artifact_url', artifact_url, '--semver_version', first_version])

    assert result.exit_code == 0

    release_model = db_session.query(Release).filter(Release.product_id == product_model.id).one()

    assert release_model

    canary_py_client = CanaryPyClient(base_url=canarypy_base_url)

    canary_py_client.send_signal_to_canary(artifact_url, first_version, instance_id, description, 'success')

    signal_model = db_session.query(Signal).filter(Signal.release_id == release_model.id).one()
